...
class GetSidebarMenu implements MenuInterface{
    ...
    private function getDiretorMenu($menuName){
        $this->getMenuFromDB($menuName, 'diretor');
    }
    private function getPoloGestorMenu($menuName){
        $this->getMenuFromDB($menuName, 'pologestor');
    }
    private function getInstrutorMenu($menuName){
        $this->getMenuFromDB($menuName, 'instrutor');
    }
    ...
    public function get($roles, $menuName = 'sidebar menu'){
        ...
        if(empty($roles)){
            ...
            $this->getDiretorMenu($menuName);
        }elseif(in_array('pologestor', $roles)){
            $this->getPoloGestorMenu($menuName);
        }elseif(in_array('instrutor', $roles)){
            $this->getInstrutorMenu($menuName);
        }
